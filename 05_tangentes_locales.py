...
- Exposes tangentes_locales API expected by tests (e.g. resolve_spectrum_path, compute_ratio_features)
  by loading the canonical implementation from exp_04.
- Preserves script execution (__main__) by delegating to impl.main() when present.
"""
...
_IMPL_PATH = (Path(__file__).resolve().parent / "experiment/exp_04_tangentes_locales/05_tangentes_locales.py").resolve()
...
# Explicit exports required by tests (fail fast if missing)
...
def __getattr__(name: str) -> Any:
    # Delegate any other attribute to implementation module
...
if __name__ == "__main__":
    # Delegate execution to impl.main() if available, otherwise do nothing.
