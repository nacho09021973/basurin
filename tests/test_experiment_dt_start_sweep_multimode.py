...
from mvp import experiment_dt_start_sweep_multimode as exp
...
def test_dt_start_sweep_writes_outputs_and_recommends_best_value(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(cmd: list[str], *, check: bool, capture_output: bool, text: bool, env: dict[str, str], cwd: str):
        ...
        dt_start_s = float(cmd[cmd.index('--dt-start-s') + 1])
        ...
        if dt_start_s == 0.003:
            ...
            _write_json(
                subrun_dir / 's4g_mode220_geometry_filter' / 'outputs' / 'mode220_filter.json',
                {'verdict': 'PASS', 'n_geometries_accepted': 30},
            )
    ...
    rc = exp.main([
        '--run-id', 'dt_sweep_host',
        '--event-id', 'GWTEST',
        '--dt-start-grid-s', '0.003,0.008',
        '--band-low', '1200',
        '--band-high', '1600',
    ...
    stage_dir = runs_root / 'dt_sweep_host' / 'experiment' / 'dt_start_sweep_multimode'
    results = json.loads((stage_dir / 'outputs' / 'dt_start_sweep_results.json').read_text(encoding='utf-8'))
    ...
    assert recommendation['recommended_dt_start']['dt_start_s'] == 0.008
    assert summary['results']['n_dt_start_values'] == 2
    assert summary['results']['overall_conclusion'] == 'FOUND_SUPPORT_REGION'
    assert (stage_dir / 'runsroot' / 'GWTEST__dt01_0p003s_band1200_1600').exists()
    assert (stage_dir / 'runsroot' / 'GWTEST__dt02_0p008s_band1200_1600').exists()


def test_dt_start_sweep_records_failed_subrun_and_stage(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runs_root = tmp_path / 'runs'
    monkeypatch.setenv('BASURIN_RUNS_ROOT', str(runs_root))
    atlas = tmp_path / 'atlas.json'
    atlas.write_text('{}', encoding='utf-8')
    ...
    rc = exp.main([
        '--run-id', 'dt_sweep_fail',
        '--event-id', 'GWFAIL',
        '--dt-start-grid-s', '0.003',
        '--band-low', '1200',
        '--band-high', '1600',
        '--atlas-path', str(atlas),
        '--offline',
    ...
    stage_dir = runs_root / 'dt_sweep_fail' / 'experiment' / 'dt_start_sweep_multimode'
    results = json.loads((stage_dir / 'outputs' / 'dt_start_sweep_results.json').read_text(encoding='utf-8'))
    recommendation = json.loads((stage_dir / 'outputs' / 'recommendation.json').read_text(encoding='utf-8'))
