...
from mvp import experiment_window_duration_sweep_multimode as exp
...
def test_window_duration_sweep_writes_outputs_and_recommends_best_value(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(cmd: list[str], *, check: bool, capture_output: bool, text: bool, env: dict[str, str], cwd: str):
        ...
        if window_duration_s == 0.06:
            ...
            _write_json(
                ...
                {
                    'combined': {'f_hz': 1200.0, 'tau_s': 1.0e-4, 'Q': 0.05},
            ...
            _write_json(
                subrun_dir / 's3_ringdown_estimates' / 'outputs' / 'estimates.json',
                {
                    'combined': {'f_hz': 1350.0, 'tau_s': 0.003, 'Q': 18.0},
                    'combined_uncertainty': {'sigma_f_hz': 8.0, 'sigma_tau_s': 2.0e-4},
                },
            )
            _write_json(
                subrun_dir / 's4g_mode220_geometry_filter' / 'outputs' / 'mode220_filter.json',
                {'verdict': 'PASS', 'n_geometries_accepted': 24},
            )
    ...
    rc = exp.main([
        '--run-id', 'window_sweep_host',
        '--event-id', 'GWTEST',
        '--window-duration-grid-s', '0.06,0.12',
        '--band-low', '1200',
        '--band-high', '1600',
    ...
    stage_dir = runs_root / 'window_sweep_host' / 'experiment' / 'window_duration_sweep_multimode'
    results = json.loads((stage_dir / 'outputs' / 'window_duration_sweep_results.json').read_text(encoding='utf-8'))
    ...
    assert recommendation['recommended_window_duration']['window_duration_s'] == 0.12
    assert summary['results']['n_window_values'] == 2
    assert summary['results']['overall_conclusion'] == 'FOUND_SUPPORT_REGION'
    assert (stage_dir / 'runsroot' / 'GWTEST__win01_0p06s_band1200_1600').exists()
    assert (stage_dir / 'runsroot' / 'GWTEST__win02_0p12s_band1200_1600').exists()


def test_window_duration_sweep_records_failed_subrun_and_stage(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    runs_root = tmp_path / 'runs'
    monkeypatch.setenv('BASURIN_RUNS_ROOT', str(runs_root))
    atlas = tmp_path / 'atlas.json'
    atlas.write_text('{}', encoding='utf-8')
    ...
    rc = exp.main([
        '--run-id', 'window_sweep_fail',
        '--event-id', 'GWFAIL',
        '--window-duration-grid-s', '0.06',
        '--band-low', '1200',
        '--band-high', '1600',
        '--atlas-path', str(atlas),
        '--offline',
    ...
    stage_dir = runs_root / 'window_sweep_fail' / 'experiment' / 'window_duration_sweep_multimode'
    results = json.loads((stage_dir / 'outputs' / 'window_duration_sweep_results.json').read_text(encoding='utf-8'))
    recommendation = json.loads((stage_dir / 'outputs' / 'recommendation.json').read_text(encoding='utf-8'))
