...
EXPERIMENT_NAME = "b5c_geometry_ranking"
...
def run(classify_run_id: str, mode: str, runs_root: str | None = None, dry_run: bool = False):
    ...
    for row in resolved:
        result = rank_geometries(row["event_run_id"], runs_root=view_root)
        per_event[row["event_id"]] = result
        for key, value in result.get("input_hashes", {}).items():
            input_hashes[f"{row['event_run_id']}::{key}"] = value
    ...
    _write_json_atomic(out_dir / "ranking_by_event.json", summary)
