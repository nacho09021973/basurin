#!/usr/bin/env python3
"""Sweep multimode pipeline over multiple dt_start_s values for one event."""
...
EXPERIMENT_STAGE = "experiment/dt_start_sweep_multimode"
RESULTS_JSON = "dt_start_sweep_results.json"
SUMMARY_CSV = "dt_start_sweep_summary.csv"
...
def _parse_dt_start_grid(raw: str) -> list[float]:
    values: list[float] = []
    seen: set[float] = set()
    for chunk in raw.split(','):
        ...
        value = float(token)
        if value < 0.0:
            raise ValueError(f'invalid dt_start_s: {token!r} (must be >= 0)')
        if value not in seen:
            values.append(value)
            seen.add(value)
    if not values:
        raise ValueError('no valid dt_start_s values parsed from --dt-start-grid-s')
    return values


def _dt_token(value: float) -> str:
...
def _subrun_id(event_id: str, dt_start_s: float, band_low: float, band_high: float, index: int) -> str:
    safe_event = _safe_event_id(event_id)
    return f"{safe_event}__dt{index:02d}_{_dt_token(dt_start_s)}s_band{_band_token(band_low)}_{_band_token(band_high)}"


def _build_cmd(args: argparse.Namespace, *, subrun_id: str, dt_start_s: float) -> list[str]:
    cmd = [
        sys.executable,
        '-m',
        'mvp.pipeline',
...
def _summarize_subrun(*, subrun_dir: Path, subrun_id: str, event_id: str, dt_start_s: float, band_low: float, band_high: float, proc: subprocess.CompletedProcess[str]) -> dict[str, Any]:
    ...
    row: dict[str, Any] = {
        ...
        'dt_start_label': _dt_token(dt_start_s),
...
def _build_recommendation(results: list[dict[str, Any]], *, event_id: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not results:
        overall = 'NO_DT_START_VALUES_EXECUTED'
        next_action = 'provide at least one valid dt_start_s value'
    elif n_nonempty > 0:
        overall = 'FOUND_SUPPORT_REGION'
        next_action = 'use the recommended dt_start_s for follow-up runs before changing the estimator'
    elif n_mode220_nonempty > 0:
        overall = 'MODE220_REGION_FOUND_BUT_FILTERED_LATER'
        next_action = 'inspect s4j/s4k for the recommended dt_start_s before changing the band'
    elif completed and n_edge_locked == len(completed):
        overall = 'LIKELY_EDGE_LOCKED_OR_TAU_FLOOR_LOCKED'
        next_action = 'dt_start sweep did not rescue the run; try changing band or estimator settings next'
    elif completed:
        overall = 'NO_SUPPORT_REGION_ACROSS_SWEEP'
        next_action = 'dt_start sweep completed without support region; compare with a different band before scaling to cohort'
    else:
        overall = 'ALL_SUBRUNS_FAILED'
        next_action = 'inspect subrun stderr and failing stage before extending the dt_start sweep'

    return {
        'schema_name': 'dt_start_sweep_multimode_recommendation',
        'schema_version': 'v1',
...
def run_dt_start_sweep(args: argparse.Namespace) -> int:
    ...
    dt_values = _parse_dt_start_grid(args.dt_start_grid_s)
    env = os.environ.copy()
    env['BASURIN_RUNS_ROOT'] = str(runsroot)

    results: list[dict[str, Any]] = []
    for index, dt_start_s in enumerate(dt_values, start=1):
        subrun_id = _subrun_id(args.event_id, dt_start_s, args.band_low, args.band_high, index)
        validate_run_id(subrun_id, runsroot)
        cmd = _build_cmd(args, subrun_id=subrun_id, dt_start_s=dt_start_s)
        print('[dt_start_sweep_multimode] $', ' '.join(shlex.quote(part) for part in cmd), flush=True)
        proc = subprocess.run(
            cmd,
            check=False,
            capture_output=True,
        ...
        results.append(row)
        print(
            '[dt_start_sweep_multimode] '
            f'dt_start_s={row["dt_start_s"]} pipeline_status={row["pipeline_status"]} '
            f'conclusion={row["conclusion_class"]} support_region_n_final={row.get("support_region_n_final")}',
            flush=True,
        )

    results_payload = {
        'schema_name': 'dt_start_sweep_multimode_results',
        'schema_version': 'v1',
        'created_utc': _utc_now_z(),
        'event_id': args.event_id,
        'host_run_id': args.run_id,
        'experiment_stage': EXPERIMENT_STAGE,
        'band_hz': [float(args.band_low), float(args.band_high)],
        'dt_start_grid_s': [float(v) for v in dt_values],
    ...
    fieldnames = [
        'event_id', 'subrun_id', 'dt_start_s', 'band_low_hz', 'band_high_hz', 'pipeline_status', 'returncode', 'error_stage',
    ...
    summary = {
        ...
        'results': {
            ...
            'recommended_dt_start': recommendation['recommended_dt_start'],
    ...
    write_manifest(
        stage_dir,
        {
            'dt_start_sweep_results': results_path,
            'dt_start_sweep_summary_csv': summary_csv_path,
            'recommendation': recommendation_path,
        },
        extra={
            'schema_name': 'dt_start_sweep_multimode_manifest',
            'subruns_root': str(runsroot),
        },
    )

    print('[dt_start_sweep_multimode] overall_conclusion=' + str(recommendation['overall_conclusion']))
    if recommendation.get('recommended_dt_start') is not None:
        selected = recommendation['recommended_dt_start']
        print(
            '[dt_start_sweep_multimode] recommended_dt_start=' +
            f"{selected.get('dt_start_s')} conclusion={selected.get('conclusion_class')}",
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description='Experiment: sweep multimode pipeline across dt_start_s values for one event')
    ap.add_argument('--run-id', required=True, help='Host run id for experiment outputs')
    ap.add_argument('--event-id', required=True)
    ap.add_argument('--dt-start-grid-s', required=True, help='Comma-separated dt_start_s values in seconds, e.g. 0,0.001,0.003,0.005')
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        return run_dt_start_sweep(args)
    except ValueError as exc:
        print(f'[dt_start_sweep_multimode] ERROR: {exc}', file=sys.stderr)
        return 2
